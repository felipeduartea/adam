...
class UniqueConstraintViolationError extends BaseError {
  static schema = z.object({
    status: z.literal(409),
    error: z.literal(ErrorCode.UNIQUE_CONSTRAINT_VIOLATION),
    message: z.string().openapi({
  ...
  constructor(message: string = "Unique constraint violation") {
    super(message, ErrorCode.UNIQUE_CONSTRAINT_VIOLATION);
  }
}
